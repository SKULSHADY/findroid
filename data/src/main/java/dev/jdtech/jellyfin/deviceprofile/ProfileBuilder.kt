package dev.jdtech.jellyfin.deviceprofile
...
class ProfileBuilder {
    ...
    ): List<SubtitleProfile> = ArrayList<SubtitleProfile>().apply {
    ...
    companion object {
        ...
        private val FORCED_AUDIO_CODECS =
            arrayOf(*PCM_CODECS, "alac", "aac", "ac3", "eac3", "dts", "mlp", "truehd")
        ...
        fun getDeviceProfile(profileBuilder: ProfileBuilder): DeviceProfile {
            ...
            for (i in SUPPORTED_CONTAINER_FORMATS.indices) {
                ...
                if (profileBuilder.supportedVideoCodecs[i].isNotEmpty()) {
                    ...
                    directPlayProfiles.add(
                        DirectPlayProfile(
                            ...
                            videoCodec = profileBuilder.supportedVideoCodecs[i].joinToString(","),
                            audioCodec = profileBuilder.supportedAudioCodecs[i].joinToString(","),
                        ),
                    )
                }
                if (profileBuilder.supportedAudioCodecs[i].isNotEmpty()) {
                    containerProfiles.add(
                        ContainerProfile(
                            type = DlnaProfileType.AUDIO,
                            container = container,
            ...
            val subtitleProfiles = profileBuilder.getSubtitleProfiles(
                EXO_EMBEDDED_SUBTITLES + SUBTITLES_SSA,
                EXO_EXTERNAL_SUBTITLES + SUBTITLES_SSA
            )

            return DeviceProfile(
                name = "Default",
                directPlayProfiles = directPlayProfiles,
                transcodingProfiles = profileBuilder.transcodingProfiles,
